...
# ------------ Job detail page ------------
def pick_info_value(soup: BeautifulSoup, title: str) -> str | None:
...
def extract_deadline(soup: BeautifulSoup) -> str | None:
...
def extract_company_link_from_job(soup: BeautifulSoup) -> str | None:
...
# ------------ Company page ------------
def scrape_company(session: requests.Session, company_url: str | None) -> dict:
    if not company_url:
        return {
...
if __name__ == "__main__":
    ...
    df = crawl_to_dataframe(
        qtpl, start_page=1, end_page=1, delay_between_pages=(0.5, 1)
    )  # thay end_page=5 nếu muốn nhiều trang hơn (5 trang)
    print(df.head())
    df.to_csv(
        "../data-files/topcv_data_analyst_jobs.csv", index=False, encoding="utf-8-sig"
    )
