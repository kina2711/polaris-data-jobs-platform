...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN posted_to_discord THEN url END) AS discord_postings,
    ROUND(CAST(COUNT(DISTINCT CASE WHEN posted_to_discord THEN url END) AS INT) / COUNT(DISTINCT url) * 100, 2) AS pct_discord_posted,
